...
// Tr!Force: [RGBSabers] Glow shaders
...
// Tr!Force: [RGBSabers] Saber color
static void UI_RGBForSaberColor( saber_colors_t color, vec3_t rgb, int bnum ) {
	...
	case SABER_RGB:
		if ( bnum == 0 ) {
			rgb[0] = atoi( UI_Cvar_VariableString( "ui_sab1_r" ) );
			rgb[1] = atoi( UI_Cvar_VariableString( "ui_sab1_g" ) );
			rgb[2] = atoi( UI_Cvar_VariableString( "ui_sab1_b" ) );
		}
		else {
			rgb[0] = atoi( UI_Cvar_VariableString( "ui_sab2_r" ) );
			rgb[1] = atoi( UI_Cvar_VariableString( "ui_sab2_g" ) );
			rgb[2] = atoi( UI_Cvar_VariableString( "ui_sab2_b" ) );
		}
		for ( i = 0; i < 3; i++ )
			rgb[i] /= 255;
		break;
...
void UI_DoSaber( vec3_t origin, vec3_t dir, float length, float lengthMax, float radius, saber_colors_t color, int rfx, qboolean doLight, int cnum, int bnum ) // Tr!Force: [RGBSabers] Blade color
{
	...
	switch( color )
	{
		...
		case SABER_PURPLE:
			...
			break;
		case SABER_RGB: // Tr!Force: [RGBSabers] Saber color
		default:
	...
	UI_RGBForSaberColor( color, rgb, bnum ); // Tr!Force: [RGBSabers] Saber color
	...
	for ( i = 0; i < 3; i++ ) rgb[i] *= 255; // Tr!Force: [RGBSabers] Saber color
	...
	saber.renderfx = rfx; // Tr!Force: [RGBSabers] Saber color
...
//	vec3_t	angles={0};
	...
	int snum; // Tr!Force: [RGBSabers] Blade number
	char *tagName;
	int bolt;
	float scale;
	...
	{
		snum = 0; // Tr!Force: [RGBSabers] Blade number
	...
	{
		snum = 1; // Tr!Force: [RGBSabers] Blade number
...
//	scale = DC->xscale;
	...
	UI_DoSaber( bladeOrigin, axis[0], bladeLength, bladeLength, bladeRadius, bladeColor, 0, qfalse, 0, snum ); // Tr!Force: [RGBSabers] Blade number
}
