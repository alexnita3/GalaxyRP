	...
	if (Q_stricmp(ent->targetname, "zyk_super_beam") == 0)
	{
		...
		{ // zyk: if the enemy is hit by the super beam, damage him
			if (user_ent->client->pers.player_statuses & (1 << 23)) // zyk: Multi Beam does less damage
				G_Damage(player_ent, user_ent, user_ent, NULL, player_ent->client->ps.origin, 12, DAMAGE_NO_PROTECTION, MOD_UNKNOWN);
			else
				G_Damage(player_ent, user_ent, user_ent, NULL, player_ent->client->ps.origin, 36, DAMAGE_NO_PROTECTION, MOD_UNKNOWN);
		}
